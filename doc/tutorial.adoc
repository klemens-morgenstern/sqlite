...
Once the `conn` is created, a common first step is to create a database schema.
This can be done with the `execute` command, which allows the execution of multiple statements with a single invocation.
...
For brevity, the tutorial section will use the exception throwing overloads. See the <<reference>> for details on the overloads.
...
The <<resultset,`resultset`>> is a forward range of the data queried from the database.
...
The <<row, `row`>> type is a range of <<field, `field`>>,
which has all the information for the field requested.

In many use-cases manual access to columns is superfluous because the structure of the table is known.

To make this easy, you can call query with a type parameter (`query<T>`) which will yield a
<<static_resultset, `static_resultset<T>`>>. `T` can either be a tuple of a struct.
...
WARNING: `blob_view` and `string_view` have a limited lifetime. They may be invalidated when the next row is read.
...
You can add type checks by using strict mode:
...
This is done by creating a <<statement,`statement`>> and then executing it with parameters.
