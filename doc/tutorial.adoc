...
NOTE: The connection is an open file-handle, not a network connection to a remove database server, as sqlite embedded in the current process.
...
For brevity, this tutorial section will use the exception overloads. See the <<reference>> for details on the overloads.
...
The <<row, row>> type is a range of <<field,fields>>,
...
To make this easy, the <<statement_range,statement_range>> has a type parameter (or `query<T>`) which will transform every row into `T`. 

`T` can either be a tuple of a struct. A tuple will assign the query results by order.
...
You can add type checks, by setting the strict mode to `true` with the second template parameter.
...
`:`, `@`, `$` are named parameters, e.g. `$age`. Named parameters can make queries more readable and provide more checks but add overhead.
...
NOTE: `execute()` is equivalent to calling `.bind()`, `.step()` and `.reset()`.
