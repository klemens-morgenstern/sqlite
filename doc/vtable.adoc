...
https://www.sqlite.org/vtab.html[A Virtual Table] is an object that is registered with a <<connection,connection>>.
...
NOTE: Constraints of the same type can appear multiple times, so the filter must only narrow the range.
